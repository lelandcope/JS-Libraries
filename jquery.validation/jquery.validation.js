/**
	...
	Ver. 1.6
		Added a postal_code validator that will validate US and Canada postal codes
	...
	$.validate = function(form)
	{
		...
		form.find('.validate').each(function()
		{
			...
			if(validation == 'postal_code') {
				var postalObj = /(^\d{5}(-\d{4})?$)|(^[ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy]{1}\d{1}[A-Za-z]{1} *\d{1}[A-Za-z]{1}\d{1}$)/;

				if(!postalObj.test($this.val())) {
					$this.parent().addClass('input-error');
					isOk = false;
				}
			}
